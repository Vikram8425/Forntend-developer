...
function findCaliforniaCafes(searchstring) {
   ...
   const results=cafes.filter(each => each.name.includes(searchstring))
         
      //const find= places.filter(el =>results.some(f =>f.place_id===el.id))

      const result=[];
       const find1= places.filter(p =>{
  ...
  return result
   ...
   const results=findCaliforniaCafes("Aven")
